...
Thanks for taking the time to contribute to `iamenr0s.ansible_role_users`!
...
2. Install the development dependencies:
   ...
   ```bash
   pip3 install ansible molecule molecule-plugins[docker] docker cryptography yamllint ansible-lint
...
Molecule uses Podman locally and Docker in CI. The CI matrix covers Debian, Ubuntu, AlmaLinux, Rocky Linux, and Fedora (see the `strategy.matrix.distro` list in `.github/workflows/molecule.yml`) — changes must not break any supported distro.
